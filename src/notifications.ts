...
type Issue = { id?: number; issue_type: string; issue_detail?: string; username: string; description: string; playback: any; created_at?: string };
const timeout = () => AbortSignal.timeout(10_000);
const trim = (value: string, max = 1900) => value.length > max ? value.slice(0, max - 1) + '…' : value;
function formatTicks(ticks: unknown) { const total = Math.max(0, Math.floor(Number(ticks) / 10_000_000)); if (!Number.isFinite(total)) return ''; const hours = Math.floor(total / 3600), minutes = Math.floor(total % 3600 / 60), seconds = total % 60; return hours ? `${hours}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}` : `${minutes}:${String(seconds).padStart(2, '0')}`; }
function content(issue: Issue) { const p = issue.playback || {}; const media = [p.seriesName, p.seasonName, p.itemName].filter(Boolean).join(' · ') || 'Unknown item'; const formatted = p.positionTicks === undefined || p.positionTicks === null ? '' : formatTicks(p.positionTicks); const seen = p.source === 'recent' && p.lastSeen ? `, last seen ${new Date(p.lastSeen).toLocaleString()}` : ''; const manualProgress = p.progressSource === 'jellyfin-history' ? [formatted ? `saved position ${formatted}` : '', p.playedPercentage != null ? `${Number(p.playedPercentage).toFixed(0)}% watched` : ''].filter(Boolean).join(', ') : '', position = p.source === 'manual' ? ` (manually selected${manualProgress ? `; Jellyfin ${manualProgress}` : ''})` : formatted ? ` ${p.source === 'recent' ? `(most recently watched; last known position ${formatted}${seen})` : `(playback time ${formatted})`}` : p.source === 'recent' ? ` (most recently watched${seen})` : ''; const title = `JellyPulse: ${issue.issue_type} issue`; const report = [issue.issue_detail, issue.description].filter(Boolean).join('\n'); const message = `${issue.username} reported ${issue.issue_type} for ${media}${position}.${report ? `\n\n${report}` : ''}`; return { title, message: trim(message) }; }

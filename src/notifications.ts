...
function content(issue: Issue) { const p = issue.playback || {}; const media = [p.seriesName, p.seasonName, p.itemName].filter(Boolean).join(' · ') || 'Unknown item'; const formatted = p.positionTicks === undefined || p.positionTicks === null ? '' : formatTicks(p.positionTicks); const seen = p.source === 'recent' && p.lastSeen ? `, last seen ${new Date(p.lastSeen).toLocaleString()}` : ''; const position = formatted ? ` ${p.source === 'recent' ? `(most recently watched; last known position ${formatted}${seen})` : `(playback time ${formatted})`}` : p.source === 'recent' ? ` (most recently watched${seen})` : ''; const title = `JellyPulse: ${issue.issue_type} issue`; const message = `${issue.username} reported ${issue.issue_type} for ${media}${position}.\n\n${issue.description}`; return { title, message: trim(message) }; }
